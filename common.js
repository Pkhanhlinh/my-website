// Language translations (excluding proper nouns: Kalynk An Tuệ, Om.Vibe, Flamingo Đại Lải)
const translations = {
    vi: {
        // Navigation
        home: "Home",
        homestay: "Homestay",
        healing: "Healing",
        villa: "Flamingo",
        healthy: "Healthy",
        // Main content
        kham_pha_title: "Các lĩnh vực mà Kalynk An Tuệ có thể kết nối cùng bạn",
        contact_info: "(whatsApp zalo: 0777998858 - 0911858998 - 0966758689)",
        homestay_title: "OM.VIBE RETREAT MIEU LAKE",
        homestay_subtitle: "Homestay cho Thân - Tâm - Trí",
        healing_title: "THIỀN THỤ ĐỘNG",
        healing_subtitle: "Bằng Âm Thanh Chữa Lành",
        villa_title: "BOOKING VILLA FLAMINGO",
        villa_subtitle: "Chính Chủ",
        healthy_title: "Trị liệu",
        healthy_subtitle: "cơ thể",
        facebook: "Facebook",
        instagram: "Instagram",
        // Founder section
        founder_title: "Lắng lòng nghe hơi thở, Thấy mình trong tinh không!",
        founder_greeting: "Chào bạn, mời bạn cứ gọi mình là Kalynk An Tuệ nhé!",
        founder_desc_1: 'Mình ghé thăm thế giới này với tư cách là một "người học việc" — học cách thương, cách hiểu và học cách lắng nghe. Thế nên, nếu bạn đang mang một túi tâm sự trĩu nặng, cứ ngồi xuống đây, mình luôn sẵn lòng nghe tiếng lòng của bạn như nghe một bản nhạc đời đầy màu sắc.',
        founder_desc_2: "Mình vốn là một kiến trúc sư ở Tây Hồ, nhưng lăn lộn đến tận Hồ Miễu để tìm bình yên vì mình không cần hồ quá rộng lớn, chỉ cần hồ đủ lặng để soi thấy chính mình.",
        founder_desc_3: 'Nghe thì có vẻ "to tát", nhưng thực ra công việc của mình giản đơn lắm: mình mời bạn ngồi xuống, thở thật khê, và để âm thanh đưa bạn vào trạng thái thiền thụ động.',
    },
    en: {
        home: "Home",
        homestay: "Homestay",
        healing: "Healing",
        villa: "Flamingo",
        healthy: "Healthy",
        kham_pha_title: "Areas Kalynk An Tuệ can connect with you",
        contact_info: "(whatsApp zalo: 0777998858 - 0911858998 - 0966758689)",
        homestay_title: "OM.VIBE RETREAT MIEU LAKE",
        homestay_subtitle: "Homestay for Body - Mind - Spirit",
        healing_title: "ACTIVE MEDITATION",
        healing_subtitle: "With Healing Sounds",
        villa_title: "BOOKING VILLA FLAMINGO",
        villa_subtitle: "Direct from Owner",
        healthy_title: "Therapy",
        healthy_subtitle: "for body",
        facebook: "Facebook",
        instagram: "Instagram",
        founder_title: "Listen to your breath, See yourself in the infinite!",
        founder_greeting: "Hello, please call me Kalynk An Tuệ!",
        founder_desc_1: 'I visit this world as a "learner" — learning to love, to understand and to listen.',
        founder_desc_2: "I was originally an architect in Tay Ho, but I rolled to Mieu Lake to find peace.",
        founder_desc_3: 'It may sound "big", but actually my work is very simple: I invite you to sit down, breathe gently.',
    },
    fr: {
        home: "Accueil",
        homestay: "Homestay",
        healing: "Guérison",
        villa: "Flamingo",
        healthy: "Santé",
        kham_pha_title: "Domaines où Kalynk An Tuệ peut vous connecter",
        contact_info: "(whatsApp zalo: 0777998858 - 0911858998 - 0966758689)",
        homestay_title: "OM.VIBE RETREAT MIEU LAKE",
        homestay_subtitle: "Homestay pour Corps - Esprit - Âme",
        healing_title: "MÉDITATION ACTIVE",
        healing_subtitle: "Avec des Sons Guérisseurs",
        villa_title: "RÉSERVATION VILLA FLAMINGO",
        villa_subtitle: "Propriétaire",
        healthy_title: "Thérapie",
        healthy_subtitle: "du corps",
        facebook: "Facebook",
        instagram: "Instagram",
        founder_title: "Écoutez votre souffle, Voyez-vous dans l'infini!",
        founder_greeting: "Bonjour, appelez-moi Kalynk An Tuệ!",
        founder_desc_1: 'Je visite ce monde en tant qu\'apprenant — apprendre à aimer, à comprendre et à écouter.',
        founder_desc_2: "J'étais à l'origine un architecte à Tay Ho, mais j'ai roulé vers le lac Mieu pour trouver la paix.",
        founder_desc_3: 'Cela peut paraître "grand", mais en fait mon travail est très simple: je vous invite à vous asseoir.',
    },
    ja: {
        home: "ホーム",
        homestay: "ホームステー",
        healing: "メロディ",
        villa: "フラミンゴ",
        healthy: "ヘルス",
        kham_pha_title: "Kalynk An Tuệが皆さんと繋がる分野",
        contact_info: "(whatsApp zalo: 0777998858 - 0911858998 - 0966758689)",
        homestay_title: "OM.VIBE RETREAT MIEU LAKE",
        homestay_subtitle: "ホームステー - 体・心・魂",
        healing_title: "能動的瞑想",
        healing_subtitle: "癒しの音で",
        villa_title: "ヴィラ フラミンゴ ご予約",
        villa_subtitle: "オーナー直営",
        healthy_title: "セラピー",
        healthy_subtitle: "ボディ",
        facebook: "Facebook",
        instagram: "Instagram",
        founder_title: "呼吸に耳を傾け、無限の中で自分を見よう！",
        founder_greeting: "こんにちは、Kalynk An Tuệと呼んでください！",
        founder_desc_1: '私は「学び手」としてこの世界を訪れています — 愛すること、を理解すること、そして聴くことを学ぶ。',
        founder_desc_2: "私は元々Tay Hoの建築家でしたが、平和を見つけるためにMieu湖まで転がり落ちました。",
        founder_desc_3: 'それは「大きく」聞こえるかもしれませんが、私の仕事は実際には非常に簡単です：座って優しく呼吸する。',
    },
    ko: {
        home: "홈",
        homestay: "홈스테이",
        healing: "힐링",
        villa: "플라밍고",
        healthy: "헬스",
        kham_pha_title: "Kalynk An Tuệ가 여러분과 연결될 수 있는 분야",
        contact_info: "(whatsApp zalo: 0777998858 - 0911858998 - 0966758689)",
        homestay_title: "OM.VIBE RETREAT MIEU LAKE",
        homestay_subtitle: "홈스테이 - 몸, 마음, 영혼",
        healing_title: "능동적 명상",
        healing_subtitle: "치유 음성으로",
        villa_title: "플라밍고 빌라 예약",
        villa_subtitle: "소유자 직영",
        healthy_title: "치료",
        healthy_subtitle: "신체",
        facebook: "Facebook",
        instagram: "Instagram",
        founder_title: "호흡에 귀 기울이고, 무한 속에서 자신을 발견하세요!",
        founder_greeting: "안녕하세요, Kalynk An Tuệ라고 불러주세요!",
        founder_desc_1: '저는 "배우는 사람"으로서 이 세상을 방문합니다 — 사랑하는 법, 이해하는 법, 그리고 경청하는 법을 배우고 있습니다。',
        founder_desc_2: "저는 원래 Tay Ho의 건축가였지만, 평화를 찾기 위해 Mieu 호수까지 굴러갔습니다。",
        founder_desc_3: '들으면 "거창한" 것처럼 들릴 수 있지만, 실제로 저의 일은 매우 간단합니다：앉아서 부드럽게 숨을 쉬는 것。',
    },
    zh: {
        home: "首页",
        homestay: "民宿",
        healing: "疗愈",
        villa: "弗拉明戈",
        healthy: "健康",
        kham_pha_title: "Kalynk An Tuệ可以与您连接的领域",
        contact_info: "(whatsApp zalo: 0777998858 - 0911858998 - 0966758689)",
        homestay_title: "OM.VIBE RETREAT MIEU LAKE",
        homestay_subtitle: "民宿 - 身心灵",
        healing_title: "主动冥想",
        healing_subtitle: "用疗愈的声音",
        villa_title: "弗拉明戈别墅预订",
        villa_subtitle: "业主直营",
        healthy_title: "治疗",
        healthy_subtitle: "身体",
        facebook: "Facebook",
        instagram: "Instagram",
        founder_title: "倾听呼吸，在无限中看见自己！",
        founder_greeting: "您好，请叫我Kalynk An Tuệ！",
        founder_desc_1: '我以"学习者"的身份来到这个世界 — 学习爱、学习理解、学习倾听。',
        founder_desc_2: "我原本是西湖的建筑师，但我漂到妙湖寻找平静。",
        founder_desc_3: '听起来可能"很宏大"，但实际上我的工作非常简单：坐下轻轻呼吸。',
    },
    ru: {
        home: "Главная",
        homestay: "Гостевой дом",
        healing: "Исцеление",
        villa: "Фламинго",
        healthy: "Здоровье",
        kham_pha_title: "Области, где Kalynk An Tuệ может с вами связаться",
        contact_info: "(whatsApp zalo: 0777998858 - 0911858998 - 0966758689)",
        homestay_title: "OM.VIBE RETREAT MIEU LAKE",
        homestay_subtitle: "Гостевой дом - Тело - Разум - Душа",
        healing_title: "АКТИВНАЯ МЕДИТАЦИЯ",
        healing_subtitle: "С Исцеляющими Звуками",
        villa_title: "БРОНИРОВАНИЕ ВИЛЛЫ ФЛАМИНГО",
        villa_subtitle: "От Владельца",
        healthy_title: "Терапия",
        healthy_subtitle: "тела",
        facebook: "Facebook",
        instagram: "Instagram",
        founder_title: "Слушайте своё дыхание, Увидьте себя в бесконечности!",
        founder_greeting: "Здравствуйте, пожалуйста, называйте меня Kalynk An Tuệ!",
        founder_desc_1: 'Я посещаю этот мир как "ученик" — учусь любить, понимать и слушать。',
        founder_desc_2: "Я изначально был архитектором в Тай Хо, но я скатился к озеру Мьеу, чтобы найти покой。",
        founder_desc_3: 'Это может звучать "громоздко", но на самом деле моя работа очень проста：сесть и мягко дышать。',
    }
};

let currentLanguage = 'vi';

// Language names in their native scripts
const languageNames = {
    vi: 'Tiếng Việt 🇻🇳',
    en: 'English 🇺🇸',
    fr: 'Français 🇫🇷',
    ja: '日本語 🇯🇵',
    ko: '한국어 🇰🇷',
    zh: '中文 🇨🇳',
    ru: 'Русский 🇷🇺'
};

function changeLanguage(lang) {
    currentLanguage = lang;
    
    // Update all translatable elements with data-i18n attribute
    document.querySelectorAll('[data-i18n]').forEach(element => {
        const key = element.getAttribute('data-i18n');
        if (translations[lang] && translations[lang][key]) {
            element.textContent = translations[lang][key];
        }
    });
    
    // Update elements with specific IDs
    const khamPhaTitle = document.querySelector('#kham-pha h2');
    const khamPhaSmall = document.querySelector('#kham-pha small');
    const founderTitle = document.querySelector('#founder h2');
    const founderGreeting = document.querySelector('#founder p');
    const founderDesc1 = document.querySelector('#founder p:nth-of-type(3)');
    const founderDesc2 = document.querySelector('#founder p:nth-of-type(4)');
    const founderDesc3 = document.querySelector('#founder p:nth-of-type(5)');
    
    // Map keys to elements
    const elementMap = {
        'kham_pha_title': khamPhaTitle,
        'contact_info': khamPhaSmall,
        'founder_title': founderTitle,
        'founder_greeting': founderGreeting,
        'founder_desc_1': founderDesc1,
        'founder_desc_2': founderDesc2,
        'founder_desc_3': founderDesc3
    };
    
    Object.keys(elementMap).forEach(key => {
        const element = elementMap[key];
        if (element && translations[lang] && translations[lang][key]) {
            element.textContent = translations[lang][key];
        }
    });
    
    // Save preference
    localStorage.setItem('preferredLanguage', lang);
    
    // Update button text
    const langBtn = document.getElementById('lang-button');
    if (langBtn) {
        langBtn.innerHTML = languageNames[lang] + ' ▼';
    }
    
    // Close dropdown after selection
    const dropdown = langBtn ? langBtn.closest('.dropdown') : null;
    if (dropdown) {
        dropdown.classList.remove('show');
    }
}

function toggleLanguageDropdown(event) {
    event.stopPropagation();
    const dropdown = event.target.closest('.dropdown');
    if (dropdown) {
        dropdown.classList.toggle('show');
    }
}

// Close dropdown when clicking outside
document.addEventListener('click', function(event) {
    if (!event.target.closest('.dropdown')) {
        document.querySelectorAll('.dropdown').forEach(dropdown => {
            dropdown.classList.remove('show');
        });
    }
});

document.addEventListener('DOMContentLoaded', function() {
    // Add header content with navigation buttons
    const headerPlaceholder = document.getElementById('header-placeholder');
    if (headerPlaceholder) {
        headerPlaceholder.innerHTML = `
<div style="display: flex; justify-content: space-between; align-items: center;">
    <div style="font-size: 3em; font-weight: 900; font-family: 'Times New Roman', 'Arial Unicode MS', Arial, sans-serif; text-transform: uppercase;">KALYNK_AN TUỆ</div>
    <div class="nav-buttons">
        <a href="index.html" class="button" data-i18n="home">Home</a>
        <a href="homestay.html" class="button" data-i18n="homestay">Homestay</a>
        <a href="healing.html" class="button" data-i18n="healing">Healing</a>
        <a href="villa.html" class="button" data-i18n="villa">Flamingo</a>
        <a href="healthy.html" class="button" data-i18n="healthy">Healthy</a>
    </div>
</div>

<!-- Mobile Header with Language Switcher -->
<div id="mobile-header" style="display: none; justify-content: space-between; align-items: center; padding: 5px 10px;">
    <div class="language-switcher-mobile"></div>
    <div style="font-size: 1.2em; font-weight: 900; font-family: 'Times New Roman', 'Arial Unicode MS', Arial, sans-serif; text-transform: uppercase; color: white;">KALYNK_AN TUỆ</div>
</div>
`;
    }

    // Check for saved language preference
    const savedLang = localStorage.getItem('preferredLanguage');
    if (savedLang && translations[savedLang]) {
        currentLanguage = savedLang;
    }

    // Add language switcher
    const khamPhaSection = document.querySelector('#kham-pha');
    const headerDiv = document.querySelector('#header-placeholder > div') || document.querySelector('header > div');
    
    if (!document.getElementById('language-switcher')) {
        const langSwitcher = document.createElement('div');
        langSwitcher.id = 'language-switcher';
        langSwitcher.innerHTML = `
            <div class="dropdown">
                <button id="lang-button" class="lang-btn" onclick="toggleLanguageDropdown(event)">
                    Tiếng Việt 🇻🇳 ▼
                </button>
                <div class="dropdown-content">
                    <a href="#" onclick="changeLanguage('vi'); return false;">Tiếng Việt 🇻🇳</a>
                    <a href="#" onclick="changeLanguage('en'); return false;">English 🇺🇸</a>
                    <a href="#" onclick="changeLanguage('fr'); return false;">Français 🇫🇷</a>
                    <a href="#" onclick="changeLanguage('ja'); return false;">日本語 🇯🇵</a>
                    <a href="#" onclick="changeLanguage('ko'); return false;">한국어 🇰🇷</a>
                    <a href="#" onclick="changeLanguage('zh'); return false;">中文 🇨🇳</a>
                    <a href="#" onclick="changeLanguage('ru'); return false;">Русский 🇷🇺</a>
                </div>
            </div>
        `;
        
        // On mobile, insert language switcher into mobile-header; on desktop, insert in header
        if (window.innerWidth <= 600) {
            const mobileHeader = document.getElementById('mobile-header');
            const langMobile = mobileHeader.querySelector('.language-switcher-mobile');
            if (langMobile) {
                langMobile.appendChild(langSwitcher);
            }
        } else if (headerDiv) {
            headerDiv.insertBefore(langSwitcher, headerDiv.firstChild);
        } else if (khamPhaSection) {
            khamPhaSection.insertBefore(langSwitcher, khamPhaSection.firstChild);
        }
        
        // Apply saved language
        changeLanguage(currentLanguage);
    }

    // Add active class to current page button
    let currentPage = window.location.pathname.split('/').pop() || 'index.html';
    let activeLink = document.querySelector(`a[href="${currentPage}"]`);
    if (activeLink) {
        activeLink.classList.add('active');
    }
});

// Make functions available globally
window.changeLanguage = changeLanguage;
window.toggleLanguageDropdown = toggleLanguageDropdown;
